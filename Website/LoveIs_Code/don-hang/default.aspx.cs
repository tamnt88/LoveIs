...

{
...

    {
        ...
        {
            var returnUrl = Request.Url != null ? Request.Url.PathAndQuery : "/don-hang";
...

        {
            ...
            if (!order.CustomerId.HasValue || order.CustomerId.Value != customerId.Value)
            {
                ShowNotFound();
                return;
            }
...

    {
        ...
        var normalized = RemoveDiacritics((status ?? string.Empty).Trim()).ToLowerInvariant();

        var step = 1;

        if (normalized.Contains("confirm") || normalized.Contains("xac"))
        ...
        else if (normalized.Contains("complete") || normalized.Contains("done") || normalized.Contains("hoan"))
