...

{
...

    {
        ...
        SystemPageSeoApplier.Apply("cart", SeoTitleLiteral, SeoMetaLiteral, "Giỏ hàng | LoveIs Store", canonical);
...

    {
...

        {
            ...
            var invalidVariantIds = cart
                .Where(c => !variantLookup.ContainsKey(c.VariantId)
                    || !variantLookup[c.VariantId].Status
                    || GetEffectivePrice(variantLookup[c.VariantId]) <= 0)
            ...
            {
                foreach (var variantId in invalidVariantIds)
                {
...
Quantity = item.Quantity,
    ...
    private void ApplyLimitMessage(List<CartService.CartItem> cart)

    {

        if (CartLimitPanel == null || CartLimitLiteral == null)
        ...
        CartLimitPanel.Visible = true;

        CartLimitLiteral.Text = string.Format(

            "Giới hạn giỏ hàng: tối đa {0} sản phẩm/đơn, tối đa {1} mỗi sản phẩm.",
