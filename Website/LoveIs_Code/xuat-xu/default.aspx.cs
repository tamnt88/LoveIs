...
public partial class OriginDefault : System.Web.UI.Page
{
    ...
    private void BindOriginPage()
    {
        using (var db = new BeautyStoryContext())
        {
            ...
            {
                OriginTitle.Text = "Xu?t x?";
                OriginSubTitle.Text = "Kh?ng t?m th?y xu?t x?.";
            ...
            OriginTitle.Text = origin != null ? origin.OriginName : "Xu?t x?";
            OriginSubTitle.Text = string.Empty;
            OriginTitleTop.Text = origin != null ? origin.OriginName : "Xu?t x?";
            ...
            string title = origin != null && !string.IsNullOrWhiteSpace(origin.SeoTitle)
                ? origin.SeoTitle
                : (origin != null ? origin.OriginName : "Xu?t x?");
            ...
            OriginProductRepeater.DataSource = products
                ...
                .Select(p => new
                {
                    ...
                    PriceLabel = priceLookup.ContainsKey(p.Id) ? priceLookup[p.Id] : "Li?n h?",
    ...
    private static string FormatPriceHtml(List<VariantLite> variants)
    {
        ...
        {
            return string.Format("<span class=\"price-old\">{0:N0} ?</span> <span class=\"price-current\">{1:N0} ?</span>", variant.Price, variant.SalePrice.Value);
        }
        return string.Format("<span class=\"price-current\">{0:N0} ?</span>", variant.Price);
    }
    ...
    {
        string originName = origin != null ? origin.OriginName : "Xu?t x?";
        var links = new List<string>
        {
            "<li class=\"breadcrumb-item\"><a href=\"/\">Trang ch?</a></li>",
            "<li class=\"breadcrumb-item\"><a href=\"/xuat-xu\">Xu?t x?</a></li>",
