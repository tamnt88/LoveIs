...
<asp:Content ID="HeadContent" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    padding: 6px 12px;
    border-radius: 999px;
    background: linear-gradient(180deg, #3A0517 0%, #5A0A22 55%, #7B1230 100%);
    color: #fff;
    border: none;
    font-weight: normal;
    font-size: 13px;
            ...
            <div class="col-lg-9">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="product-info">
                                ...
                                <div class="product-meta">
                                    <asp:PlaceHolder ID="ProductRatingWrap" runat="server">
                                        <span class="rating-badge"><asp:Literal ID="ProductRatingLiteral" runat="server" />&#9733;</span>
                                        <span><asp:Literal ID="ProductRatingCountLiteral" runat="server" /></span>
                                        <span class="meta-sep">|</span>
                                    </asp:PlaceHolder>
                                    <asp:PlaceHolder ID="ProductNoRatingWrap" runat="server" Visible="false">
                    ...
                    <div class="flex-grow-1">
                        <div class="shop-summary-head">
                            <div class="shop-summary-title">
                                <a class="shop-name-link" href="<%= ShopUrl %>" title="Xem shop">
                                    <asp:Literal ID="ShopNameLiteral" runat="server" />
                                </a>
                            </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div id="ShopReviewErrorWrap" runat="server">
                            ...
                            <div class="modal-footer">
                                ...
                                <asp:LinkButton ID="SubmitShopReviewButton" runat="server" CssClass="btn btn-primary" OnClick="SubmitShopReviewButton_Click" OnClientClick="return validateShopReview();">Gửi đánh giá</asp:LinkButton>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="modal fade shop-review-modal" id="productReviewModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <div class="fw-semibold mb-2">Mức độ hài lòng</div>
                                    <div class="d-flex gap-2 shop-review-stars" id="productReviewStars">
                                        <button type="button" class="btn btn-outline-dark btn-sm" data-rating="1">&#9733;</button>
                                        <button type="button" class="btn btn-outline-dark btn-sm" data-rating="2">&#9733;</button>
                                        <button type="button" class="btn btn-outline-dark btn-sm" data-rating="3">&#9733;</button>
                                        <button type="button" class="btn btn-outline-dark btn-sm" data-rating="4">&#9733;</button>
                                    ...
                                    <asp:HiddenField ID="ProductReviewRatingHidden" runat="server" />
                                </div>
                                <div class="mb-3">
                                    <label for="<%= ProductReviewContentTextBox.ClientID %>" class="form-label">Nội dung</label>
                                    <asp:TextBox ID="ProductReviewContentTextBox" runat="server" TextMode="MultiLine" Rows="4" CssClass="form-control" />
                                </div>
                                <div id="ProductReviewErrorWrap" runat="server">
                                    <asp:Literal ID="ProductReviewErrorLiteral" runat="server" />
                                </div>
                                <asp:HiddenField ID="ProductReviewModalState" runat="server" />
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Đóng</button>
                                <asp:LinkButton ID="SubmitProductReviewButton" runat="server" CssClass="btn btn-primary" OnClick="SubmitProductReviewButton_Click" OnClientClick="return validateProductReview();">Gửi đánh giá</asp:LinkButton>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="product-review-section mt-4" id="product-reviews">
                    <div class="section-heading d-flex align-items-center justify-content-between">
                        <div>
                            <h4>Đánh giá</h4>
                            <p>Chia sẻ trải nghiệm mua sắm của bạn</p>
                        </div>
                        <button type="button" class="review-tag-btn" data-bs-toggle="modal" data-bs-target="#productReviewModal">
                            <i class="fa-solid fa-star"></i><span>Gửi đánh giá</span>
                        </button>
                    </div>
                    <asp:PlaceHolder ID="ProductReviewEmpty" runat="server" Visible="false">
                        <div class="bg-white border p-3">Chưa có đánh giá cho sản phẩm này.</div>
                    </asp:PlaceHolder>
                    <asp:Repeater ID="ProductReviewRepeater" runat="server">
                        <ItemTemplate>
                            <div class="bg-white border p-3 mb-2">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    <div class="fw-semibold"><%# Eval("CustomerName") %></div>
                                    <div class="text-muted small"><%# Eval("CreatedAt") %></div>
                                </div>
                                <asp:PlaceHolder ID="ReviewStatusWrap" runat="server" Visible='<%# Eval("ShowStatus") %>'>
                                    <div class="small text-muted mb-2"><%# Eval("StatusText") %></div>
                                </asp:PlaceHolder>
                                <div class="text-warning mb-2">
                                    <%# Eval("RatingStars") %>
                                </div>
                                <div><%# Eval("Content") %></div>
                                <asp:PlaceHolder ID="ReviewDeleteWrap" runat="server" Visible='<%# Eval("CanDelete") %>'>
                                    <div class="mt-2">
                                        <asp:LinkButton ID="DeleteReviewButton" runat="server" CssClass="btn btn-sm btn-outline-danger" CommandName="DeleteReview" CommandArgument='<%# Eval("Id") %>' OnCommand="DeleteProductReviewButton_Command"><i class="fa-solid fa-trash"></i><span>&nbsp;Xóa đánh giá</span></asp:LinkButton>
                                    </div>
                                </asp:PlaceHolder>
                            </div>
                        </ItemTemplate>
...
<asp:Content ID="PageScripts" ContentPlaceHolderID="PageScripts" runat="server">
    ...
    <script>
        function showShopReviewClientError(message) {
            var errorWrap = document.getElementById("<%= ShopReviewErrorWrap.ClientID %>");
            if (errorWrap) {
                errorWrap.innerHTML = "";
                var alertBox = document.createElement("div");
                alertBox.className = "alert alert-danger";
                alertBox.textContent = message;
                errorWrap.appendChild(alertBox);
            }
            var modalEl = document.getElementById("shopReviewModal");
            if (modalEl && window.bootstrap && window.bootstrap.Modal) {
        ...
        function validateShopReview() {
            var ratingHidden = document.getElementById("<%= ShopReviewRatingHidden.ClientID %>");
            var contentBox = document.getElementById("<%= ShopReviewContentTextBox.ClientID %>");
            var rating = ratingHidden ? parseInt(ratingHidden.value || "0", 10) : 0;
            var content = contentBox ? (contentBox.value || "").trim() : "";
            if (!rating || rating < 1) {
                showShopReviewClientError("\u0056ui l\u00f2ng ch\u1ecdn s\u1ed1 sao.");
                return false;
            }
            if (!content) {
                showShopReviewClientError("\u0056ui l\u00f2ng nh\u1eadp n\u1ed9i dung \u0111\u00e1nh gi\u00e1.");
                return false;
            }
            return true;
        }
        ...
        (function () {
            var submitButton = document.getElementById("<%= SubmitShopReviewButton.ClientID %>");
            if (submitButton) {
                submitButton.addEventListener("click", function (event) {
                    if (!validateShopReview()) {
                        event.preventDefault();
                        event.stopPropagation();
                    }
                });
    ...
    <script>
        (function () {
            var starWrap = document.getElementById("productReviewStars");
            var ratingHidden = document.getElementById("<%= ProductReviewRatingHidden.ClientID %>");
            if (starWrap && ratingHidden) {
                var currentRating = parseInt(ratingHidden.value || "0", 10) || 0;
                starWrap.querySelectorAll("button[data-rating]").forEach(function (btn) {
                    var initValue = parseInt(btn.getAttribute("data-rating"), 10) || 0;
            ...
            var modalState = document.getElementById("<%= ProductReviewModalState.ClientID %>");
            if (modalState && modalState.value === "open") {
                var modalEl = document.getElementById("productReviewModal");
                if (modalEl && window.bootstrap && window.bootstrap.Modal) {
                    new window.bootstrap.Modal(modalEl).show();
                }
            }
    ...
    <script>
        function showProductReviewClientError(message) {
            var errorWrap = document.getElementById("<%= ProductReviewErrorWrap.ClientID %>");
            if (errorWrap) {
                errorWrap.innerHTML = '<div class="alert alert-danger">' + message + '</div>';
            }
            var modalEl = document.getElementById("productReviewModal");
            if (modalEl && window.bootstrap && window.bootstrap.Modal) {
                if (!modalEl.classList.contains("show")) {
        ...
        function validateProductReview() {
            var ratingHidden = document.getElementById("<%= ProductReviewRatingHidden.ClientID %>");
            var contentBox = document.getElementById("<%= ProductReviewContentTextBox.ClientID %>");
            var rating = ratingHidden ? parseInt(ratingHidden.value || "0", 10) : 0;
            var content = contentBox ? (contentBox.value || "").trim() : "";
            if (!rating || rating < 1) {
                showProductReviewClientError("Vui lòng chọn số sao.");
                return false;
            }
            if (!content) {
                showProductReviewClientError("Vui lòng nhập nội dung đánh giá.");
                return false;
            }
            return true;
        }
